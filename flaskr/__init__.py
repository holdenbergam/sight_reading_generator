    ...
    def shortenurl():
        if request.method == 'POST':
            ...
            if mode_ == "sightreading":
                ...
                make_random_music(mode_, time_,key,measures,instrument,level,save_to='static/image.pdf')

                #music = make_random_music(time, key, instrument)
                #image = make_image(music)

                return render_template('result.html', mode=mode_, time=time_, key=key, measures=measures, instrument=instrument, level=level, image_name=image_name, music_name=music_name)
            elif mode_ == "musiclibrary":
                music_library()
